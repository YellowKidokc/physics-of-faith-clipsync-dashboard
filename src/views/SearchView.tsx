...
const DEFAULT_URL = 'https://search.dlowehomelab.com/';
...
export function SearchView() {
  ...
  const runSearch = () => {
    ...
    setSrc(`${DEFAULT_URL}search?q=${encodeURIComponent(q)}`);
  };

  return (
    <div style={css.container}>
      <div style={css.header}>
        <span style={css.title}>SEARCH</span>
        <input
          style={css.input}
          value={query}
          onChange={e => setQuery(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') runSearch(); }}
          placeholder="search.dlowehomelab.com — query..."
        />
        <button style={css.btn} onClick={runSearch}>SEARCH</button>
        <button style={css.btn} onClick={() => { setSrc(DEFAULT_URL); setReloadKey(k => k + 1); }}>HOME</button>
